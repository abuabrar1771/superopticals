...
import ClFDd from "../components/CLFDd.jsx";
...
const ContactLenses = () => {
  ...
  const clglassesProducts = useMemo(() => {
    return products.filter((item) => item.category === "CONTACT_LENS");
  }, [products]);
    ...
    <>
      ...
      <div>
        <p className="lg:ml-6 lg:px-20 mt-5 lg:text-3xl sm:text-xl">
          Contact Lenses at Affordable Prices
        </p>
      </div>
        ...
        <div className="relative w-full rounded-xl overflow-hidden">
          ...
          {/* <div className="absolute bottom-6 inset-0 flex items-end lg:justify-center sm:justify-end text-orange-800 sm:pr-8">
            <div className="text-center sm:text-right">
              <h2 className="text-2xl sm:text-sm md:text-xl font-semibold text-blue-800">Clear Sight Clear Mind</h2>
              <p className="text-sm sm:text-lg md:text-xl px-5 text-blue-800">Contact Lneses for Comfort & Clearity</p>
            </div>
         </div> */}
        </div>
      </div>
          ...
          <main className="w-full mt-6 lg:mt-0 self-start">
            ...
            {isFilterActive || isSearchActive ? (
              ...
              <div className="space-y-16">
                {Object.entries(groupByShape(clglassesProducts)).map(
                  ([category, items]) => (
                    <div key={category}>
